...
scraper + static HTML frontend that tracks public government meetings across 14 South
Florida municipalities on 9 distinct platforms (CivicPlus, Legistar, SharePoint/PDF,
Granicus OpenCities, MuniCode, WordPress x2, CivicClerk, Granicus ViewPublisher,
AgendaCenter). The project also now has a client-side "new records" notification
system (banner/badge + a cosmetic single-user profile switcher) - see "New Records
Notification Feature" below. The person you're working with is non-technical-ish but
...
user's own CI logs. See "Key Methodological Lessons" below before writing any new
scraper - it covers real mistakes made (and caught) across sessions that are worth not
repeating.
...
meetings, normalizes dates/times to ISO format, diffs the fresh scrape against the
previous run to detect new/modified/removed meetings (writing `changes.json`), and
writes `data.json`. `index.html` is a static frontend (FullCalendar 6 + vanilla JS)
that reads `data.json` and `changes.json` and renders a searchable calendar/list view
plus the update-notification banner, hosted directly from the repo (e.g. GitHub Pages).

**Files to re-upload to the new chat for full context:** the current `scraper.py`,
`index.html`, and (if it exists yet) `changes.json` from this repo - this document
summarizes their state and history but doesn't reproduce the full code.
...
| **Village of North Palm Beach** | **AgendaCenter (`village-npb.org/AgendaCenter`)** | **Own function, `scrape_north_palm_beach()` - reuses JIC's three-strategy row parser (tr/li/div), see below** | ⚠️ **First-pass draft — platform identified only via search-result agenda-file URL pattern (`AgendaCenter/ViewFile/Agenda/_MMDDYYYY-XXXX`), not a raw-HTML fetch; `village-npb.org` robots.txt blocks this session's fetch tool entirely. Never confirmed against a real GitHub Actions log. Real risk: CivicPlus AgendaCenter installs often load per-board meeting lists via AJAX (a "Select a Category" dropdown) rather than including every board in the initial HTML - if the run only surfaces Village Council and misses Planning Commission / Planning-Zoning-Adjustment, that's the likely cause, not a parser tuning issue.** |
...
- **`fetch_hardened_retry()` helper (added this session)**: wraps `fetch_hardened()`
  with retries across a short list of curl_cffi browser TLS fingerprints and a brief
...
### City of Palm Beach (added this session — real API + real agenda-link pattern confirmed)
...
### Town of Jupiter (added this session — UNCONFIRMED, zero network access at all)
...
### City of Riviera Beach (added this session — real markup confirmed by user, sandbox execution still unconfirmed)
...
### Town of Juno Beach (added this session — real iframe target confirmed, table markup still unconfirmed)
...
### Town of Jupiter Inlet Colony (added in follow-up session — CONFIRMED WORKING)
...
### Town of Manalapan (added this session — UNCONFIRMED, see Goals for Next Session)
...
### Town of Gulf Stream (added this session — UNCONFIRMED, see Goals for Next Session)
...
## New Records Notification Feature (added this session)

A client-side "banner/badge" system that tells the app's user(s) what's changed since
their last visit - added meetings, modified meetings (time change, agenda just
posted), and removed meetings. No real backend, no real authentication - this is a
single-user (or small handful of browsers) app, so identity is handled cosmetically.

### Design decisions made this session

- **Identity**: no real login. A "profile switcher" badge in the top-right of the
  header defaults to a cosmetic "Alissa" profile (stored in `localStorage`, not any
  kind of auth) with a one-click switch to a generic "Guest" profile, for the rare
  case someone else uses the same browser. Each profile has its own namespaced
  `lastSeen_<profile>` key so switching profiles doesn't clobber the other's
  "already seen" state.
- **What counts as "new"**: both brand-new meetings AND modifications - specifically
  time changes and `has_agenda` flipping `false -> true` (an agenda getting posted is
  arguably the single most useful "update" for this app's actual use case). Removed
  meetings (cancellations) are also tracked and surfaced, tagged distinctly.
- **Delivery, phased**: Phase 1 (built this session) is an in-app banner/badge only -
  zero new infrastructure, pure client-side plus one scraper change. Phase 2 (real
  browser push notifications) was explicitly deferred - it would require adding a
  small serverless piece (to store the browser's push subscription and let GitHub
  Actions trigger sends via VAPID/`web-push`), which the user chose not to add yet.
- **One-time feature teaser**: since this UI element is brand new, a small popover
  pointing at the profile badge announces the feature. It reappears on every visit
  until the user explicitly clicks "Don't show this again" (clicking "Got it" or just
  navigating away does NOT permanently dismiss it - only the explicit dismiss does).
- **Marking things "seen"**: expanding the banner marks everything in it as seen
  (updates `lastSeen_<profile>` to now). This is a **reversible design choice, not a
  confirmed final decision** - a per-item acknowledgment model (dismiss each update
  individually) was raised as an alternative and not fully resolved. Revisit if the
  current "expand = mark all seen" behavior feels wrong in practice.

### Backend implementation (`scraper.py`)

- **New primary key for matching a meeting across runs**: `(muni_short, date,
  normalized_title)` - deliberately excludes time, via `compute_change_key()`. This
  means a meeting whose *time* changes is detected as one "modified" event rather than
  looking like an unrelated add+remove pair.
- **Known trade-off, not yet resolved**: date IS part of the key, so a meeting that
  gets rescheduled to a **different date** currently shows up as a "removed" (old
  date) + "added" (new date) pair, not a single "date changed" entry. Date can't be
  dropped from the key without risking false matches between distinct occurrences of
  the same recurring meeting (e.g. three separate "City Commission Meeting" events on
  three different dates would otherwise collide onto one key). A smarter
  nearest-date fuzzy-match pass could pair reschedules up later if this becomes a real
  annoyance in practice - ask the user before building it, don't assume it's wanted.
- **New functions**: `_normalize_title_for_key()` (lowercase, collapse whitespace,
  strip trailing punctuation - so trivial formatting differences don't falsely count
  as a different meeting), `compute_change_key()`, `load_previous_events()` (reads the
  *previous* `data.json` before it gets overwritten; returns `[]` on missing/corrupt
  file rather than blocking the run), `diff_events(old, new)` (returns a list of
  `{type: "added"|"modified"|"removed", muni_short, muni_full, title, date, time,
  fields_changed?}` dicts), and `update_changes_log()` (appends this run's changes to
  a rolling `changes.json`, pruning entries older than 45 days, each entry stamped
  with `detected_at` in UTC ISO format).
- **`main()` change**: loads the previous `data.json` via `load_previous_events()`
  *before* the fresh scrape overwrites it, calls `diff_events()` + `update_changes_log()`,
  logs a one-line summary (counts of added/modified/removed), then proceeds to write
  `data.json` exactly as before. No scraper function itself was touched.
- **Existing `id` field is NOT used for this** - worth flagging as a separate latent
  bug found while building this: every scraper's `"id"` field is built with Python's
  built-in `hash()` on a link or title+time string, and Python randomizes string hash
  values per process by default (confirmed: two separate `python3 -c "print(hash(...))"`
  invocations gave different numbers for the identical string). That means `id` is
  **not stable across runs** and would have been useless as a diff key. It's currently
  unused anywhere in `index.html` either, so it's dead weight today - not fixed this
  session since it wasn't asked for, but worth a heads-up if anything ever starts
  relying on it as a stable identifier.
- **Logic smoke-tested** against real entries pulled from the actual `data.json` in
  this repo (not a synthetic fixture) - correctly detected a time change, a
  `has_agenda` flip, a genuine addition, and a genuine removal, and correctly left an
  unchanged event alone.

### Frontend implementation (`index.html`)

- **Profile switcher**: badge in the header (`#profileBtn` / `#profileMenu`), toggles
  between "Alissa" (avatar "A") and "Guest" (avatar "G"), backed by
  `localStorage.currentProfile`.
- **One-time teaser**: `#featureTeaser` popover anchored under the profile badge,
  gated by `localStorage.featureTeaserDismissed`.
- **Updates banner**: `#updatesBanner` above the search/filter controls. Fetches
  `changes.json` on load (`fetchChanges()`), compares each entry's `detected_at`
  against `lastSeen_<profile>` (via `getLastSeen()`, which also seeds a first-ever-visit
  baseline of "now" so a brand-new profile isn't flooded with the entire historical
  backlog), and shows a count + expandable list (`renderUpdatesBanner()`,
  `describeChange()`). Clicking the banner expands it and calls `markAllSeen()`.
- All new CSS matches the project's existing hand-rolled variables (`--primary`,
  `--card-bg`, `--text-muted`, `--border`, `--radius`) rather than introducing a new
  design system - no new dependencies were added.
- `changes.json` does not exist yet in this repo (no real run has happened since this
  feature was added) - the banner correctly shows nothing until it does; this is
  expected, not a bug, on the very first run after deploying this change.
...
00. **Confirm the new records notification feature end-to-end on a real run.**
    ...
    real GitHub Actions run yet - only against hand-picked real entries from the
    existing `data.json` in a local smoke test. Ask the user to run the workflow twice
    in a row (so there's a real "before" and "after" `data.json`) and confirm
    `changes.json` gets created with sensible content, and that the banner in
    `index.html` picks it up correctly in a real browser. Also revisit the two open
    design questions flagged in "New Records Notification Feature" above: (a) whether
    date-reschedules showing as add+remove instead of one "date changed" entry is
    actually a problem in practice, and (b) whether "expand banner = mark all seen" is
    the right model or a per-item acknowledgment is wanted instead.
